...
class COCOeval:
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            stats[12] = np.mean(self.eval['f1'][self.eval['f1'] > -1])

            stats[13] = np.mean(self.eval['precision'][self.eval['precision'] > -1])
            stats[14] = np.mean(self.eval['recall'][self.eval['recall'] > -1])
            return stats
...
def evalSemantic(coco,res_json):
    ...
    for imgId in imgIds:
        ...
        if len(ann) == 0:
            label = np.zeros([800,800])
            print("Empty GT",imgId)
            # continue
        else:
            ann = ann[0]
