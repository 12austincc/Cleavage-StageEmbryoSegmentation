...
class EarlyStopping:
    ...
    def save_checkpoint(self, score, model, epoch, fabric):
        ...
        if fabric.global_rank == 0:
            torch.save(state_dict, os.path.join(self.save_path, f"epoch-{epoch:06d}-score{score:.2f}-ckpt.pth")) 
        

    def get_checkpoint_files(self):
        checkpoint_files = glob.glob(os.path.join(self.save_path, "*-ckpt.pth"))
        # f1 = [float(file.split('-f1')[1].split('-')[0]) for file in checkpoint_files]
        # ckpt_f1_tuples = list(zip(checkpoint_files, f1))
        #sorted by f1 ascending little->big
