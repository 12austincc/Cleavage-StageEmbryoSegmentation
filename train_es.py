...
def validate(fabric: L.Fabric, model: Model, val_dataloader: DataLoader, early_stop: EarlyStopping = None,
    ...
    with torch.no_grad():
        ...
        for iter, data in enumerate(val_dataloader):
            ...
            dice_sem = diceloss(sem_pred_mask,sem_mask)
    ...
    model.to('cuda')
    score = (1-sem_dices.avg) + dices.avg
    # score = 1 - sem_dices.avg
...
def train_sam(
    ...
    for epoch in range(1, cfg.num_epochs):
        ...
        for iter, data in enumerate(train_dataloader):
            ...
            # if torch.all(sem_mask == 0) and not torch.all(sem_pred_mask == 0):
            #     loss_dice_sem = torch.Tensor(0.0, requires_grad = True)
            # else:
        ...
        fabric.log_dict(
            {"focal_loss": focal_losses.val, "dice_loss": dice_losses.val, "sem_dice_loss": dice_losses_sem.val,
             "iou_loss": iou_losses.val, "total_loss": total_losses.val},
            step=epoch
        )
...
def configure_opt(cfg: Box, model: Model):
    ...
    parameters = [
        ...
        {'params': model.semantic_decoder.parameters(), 'lr': cfg.opt.learning_rate_sem}
    ]
